...
import (
	api2 "github.com/ironcore-dev/libvirt-provider/api"
...
var _ = Describe("Store", func() {
	...
	It("should correctly create a object", func(ctx SpecContext) {
		...
		machine, err := machineStore.Create(ctx, &api2.Machine{
			Metadata: api2.Metadata{
				ID: "test-id",
			},
			Spec:   api2.MachineSpec{},
			Status: api2.MachineStatus{},
		})
		Expect(err).NotTo(HaveOccurred())
		...
		event := &store.WatchEvent[*api2.Machine]{
