...
var (
	...
	providerHost      providerhost.LibvirtHost
...
var _ = BeforeSuite(func() {
	...
	providerHost, err = providerhost.NewLibvirtAt(rootDir, libvirt)
	Expect(err).NotTo(HaveOccurred())

	By("setting up machine store")
	machineStore, err := providerhost.NewStore(providerhost.Options[*api.Machine]{
	...
	eventRecorder = machineEvent.NewEventStore(log, machineEvent.EventStoreOptions{
	...
	imgCache, err := oci.NewLocalCache(log, reg, ociStore, nil)
	Expect(err).NotTo(HaveOccurred())
	...
	networkPlugin, _, err = pluginOpts.NetworkInterfacePlugin()
	Expect(err).NotTo(HaveOccurred())
	Expect(networkPlugin.Init(context.Background(), providerHost)).To(Succeed())
