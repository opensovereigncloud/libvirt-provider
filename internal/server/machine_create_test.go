...
const (
	osImage = "ghcr.io/ironcore-dev/ironcore-oci/gardenlinux:rootfs-dev-20231206-v1"
)

var _ = Describe("CreateMachine", func() {
	It("should create a machine without boot oci, volume and network interface", func(ctx SpecContext) {
		By("creating a machine without boot oci, volume and network interface")
	...
	It("should create a machine without boot oci", func(ctx SpecContext) {
		By("creating a machine without boot oci")
		createResp, err := machineClient.CreateMachine(ctx, &iri.CreateMachineRequest{
			Machine: &iri.Machine{
				Metadata: &irimeta.ObjectMetadata{
					Labels: map[string]string{
	...
	It("should create a machine with boot oci and single empty disk", func(ctx SpecContext) {
		By("creating a machine with boot oci and single empty disk")
		createResp, err := machineClient.CreateMachine(ctx, &iri.CreateMachineRequest{
			Machine: &iri.Machine{
				Metadata: &irimeta.ObjectMetadata{
					Labels: map[string]string{
	...
	It("should create a machine with boot oci and multiple empty disks", func(ctx SpecContext) {
		By("creating a machine with boot oci and multiple empty disks")
		createResp, err := machineClient.CreateMachine(ctx, &iri.CreateMachineRequest{
			Machine: &iri.Machine{
				Metadata: &irimeta.ObjectMetadata{
					Labels: map[string]string{
