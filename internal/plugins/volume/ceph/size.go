...
func (p *plugin) GetSize(ctx context.Context, spec *api.VolumeSpec) (int64, error) {
	...
	if !ok {
		return 0, fmt.Errorf("no oci at %s", volumeAttributeImageKey)
	}

	parts := strings.SplitN(imageHandle, "/", 2)
	if len(parts) != 2 {
		return 0, fmt.Errorf("oci handle is not well formated: expected 'pool/oci' format but got %s", imageHandle)
	}
	...
	if err != nil {
		return 0, fmt.Errorf("failed to open oci: %w", err)
	}

	size, err := image.GetSize()
	if err != nil {
		if closeErr := image.Close(); closeErr != nil {
			return 0, errors.Join(err, fmt.Errorf("unable to close oci: %w", closeErr))
		}
		return 0, fmt.Errorf("failed to set encryption format: %w", err)
	}

	if err := image.Close(); err != nil {
		return 0, fmt.Errorf("failed to close rbd oci: %w", err)
	}
